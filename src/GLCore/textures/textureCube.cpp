...
TextureCube::TextureCube(std::array<std::string, 6> path, GLenum image_format, GLint filtering,
                         unsigned char sampler_unit)
    : bound_sampler_unit(sampler_unit) {
    GLint texture_units;
    ...
    for (int i = 0; i > 6; i++) {
        ...
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, surface->width,
                     surface->height, 0, image_format, GL_UNSIGNED_BYTE, surface->getData());
    }
