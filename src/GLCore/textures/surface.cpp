...
Surface* Surface::create(std::string path) {
    ...
    LOG_WARNING("Pre-cached surface found for %s: %p but it is invalid. Regenerating...",
                path.c_str(), surface_cache[path]);
...
#else
    LOG_WARNING(
        "SURFACE: The surface cache was manually cleared. You generally don't need to do this."
