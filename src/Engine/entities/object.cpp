...
Object::Object(Vertices v, Indices i, std::vector<int> slices)
    : v(std::move(v)), indexSize(i.size()), renderMode(0xF) {
...
Object::Object(Vertices v, unsigned char renderMode, std::vector<int> slices)
    : v(std::move(v)), indexSize(0), renderMode(renderMode) {
    LOG_INFO("New object created at memory address 0x%p.", this);
    vao = VAO();
    vao.bind();
    if (slices.empty()) {
