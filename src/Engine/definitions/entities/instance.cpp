...
{
	Program::current_program->setMat4(Program::current_program->metadata.model_mat_name, calculateModelMat());
	obj->render();
	if (POST_RENDER_UNINIT_MODEL_MAT)
	{
		Program::current_program->setMat4(Program::current_program->metadata.model_mat_name, std::move(glm::mat4(1.0f)));
	}
}
